...
void pid_load_from_eeprom(void) {
    ...
    /* 读取X轴PID参数 */
    at24cxx_read(PID_X_KP_ADDR, buf, 4);
    kp_x = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];

    at24cxx_read(PID_X_KI_ADDR, buf, 4);
    ki_x = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];

    at24cxx_read(PID_X_KD_ADDR, buf, 4);
    kd_x = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];

    /* 读取Y轴PID参数 */
    at24cxx_read(PID_Y_KP_ADDR, buf, 4);
    kp_y = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];

    at24cxx_read(PID_Y_KI_ADDR, buf, 4);
    ki_y = (buf[0] << 24) | (buf[1] << 16) | (buf[2] << 8) | buf[3];

    at24cxx_read(PID_Y_KD_ADDR, buf, 4);
...
void pid_save_to_eeprom(void) {
    ...
    buf[2] = (kp_x >> 8) & 0xFF;
    buf[3] = kp_x & 0xFF;
    at24cxx_write(PID_X_KP_ADDR, buf, 4);

    buf[0] = (ki_x >> 24) & 0xFF;
    buf[1] = (ki_x >> 16) & 0xFF;
    buf[2] = (ki_x >> 8) & 0xFF;
    buf[3] = ki_x & 0xFF;
    at24cxx_write(PID_X_KI_ADDR, buf, 4);

    buf[0] = (kd_x >> 24) & 0xFF;
    buf[1] = (kd_x >> 16) & 0xFF;
    buf[2] = (kd_x >> 8) & 0xFF;
    buf[3] = kd_x & 0xFF;
    at24cxx_write(PID_X_KD_ADDR, buf, 4);
    ...
    buf[2] = (kp_y >> 8) & 0xFF;
    buf[3] = kp_y & 0xFF;
    at24cxx_write(PID_Y_KP_ADDR, buf, 4);

    buf[0] = (ki_y >> 24) & 0xFF;
    buf[1] = (ki_y >> 16) & 0xFF;
    buf[2] = (ki_y >> 8) & 0xFF;
    buf[3] = ki_y & 0xFF;
    at24cxx_write(PID_Y_KI_ADDR, buf, 4);

    buf[0] = (kd_y >> 24) & 0xFF;
    buf[1] = (kd_y >> 16) & 0xFF;
    buf[2] = (kd_y >> 8) & 0xFF;
    buf[3] = kd_y & 0xFF;
    at24cxx_write(PID_Y_KD_ADDR, buf, 4);
